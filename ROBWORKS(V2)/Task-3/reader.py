...
def read_schedules(schedules_path: str, robot_ids: list[str], task_ids: list[str]) -> list[dict]:
    '''
        Description: Reads the validated schedules from files.

        Arguments: 
        - schedules_path : Path of schedules file.
        - robot_ids : Ids of robots
        - task_ids : Ids of valid tasks

        Returns:
        - list schedule dictionaries
    '''
    ...
    with open(schedules_path, 'r') as file:
        for line in file:
            ...
            if len(parts) < 3:
                print("Warning: Invalid schedule row", file = sys.stderr)
                continue
            
            schedule_id = parts[0]
            robot_id = parts[1]
            task_list = [t.strip() for t in parts[2:]]

            if robot_id not in robot_ids:
                print(f"Warning: Schedule {schedule_id} invalid robot", file = sys.stderr)
                continue

            if any(t not in task_ids for t in task_list):
                print(f"Warning: Schedule {schedule_id} invalid robot", file = sys.stderr)
                continue
...
def read_distances(distances_path: str) -> list[list[float]]:
    '''
        Description: Reads distance matrix from a valid CSV file.

        Arguments: 
        - distances_path: file path of distance CSV file.

        Returns:
        - 2 Dimentional list of all float values
    '''
